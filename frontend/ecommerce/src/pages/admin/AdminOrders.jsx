...
export default function AdminOrders() {
  ...
  useEffect(() => {
    async function fetchOrder() {
      try {
        const order = await Request("orders").getAll();
        console.log("orders", order);
        if (order) setOrders(order.data);
      } catch (err) {
        console.log("Error to find orders", err);
      }
    }
    fetchOrder();
  }, []);
                  ...
                  <td className="px-6 py-3.5">
                    <div className="relative inline-block">
                      <select
                        ...
                        className={`text-xs font-semibold px-3 py-1.5 rounded-full appearance-none cursor-pointer pr-7 capitalize ${
