...
export default function AdminReviews() {
  ...
  useEffect(() => {
    async function fetchComments() {
      try {
        const res = await Request("comments").getAll();
        console.log("comments are", res.data);
        if (res.data) setComments(res.data);
      } catch (err) {
        console.log("Error to load comments.", err);
      }
    }
    fetchComments();
  }, []);
  const filtered =
    filter === "All"
      ? comments
      : comments.filter((r) => r.rating === parseInt(filter));

  const avgRating = (
    comments.reduce((sum, r) => sum + r.rating, 0) / comments.length
  ).toFixed(1);

  const ratingCounts = [5, 4, 3, 2, 1].map(
    (r) => comments.filter((rev) => rev.rating === r).length,
  );
              ...
              <div key={stars} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-3 bg-slate-100 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${(ratingCounts[idx] / comments.length) * 100}%`,
                    }}
                  ></div>
                </div>
